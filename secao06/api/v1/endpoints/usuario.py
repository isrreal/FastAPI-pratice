...
async def post_usuario(usuario: UsuarioSchemaCreate, db: AsyncSession = Depends(get_session)):
    ...
    async with db as session:
        try:
            ...
            raise HTTPException(status_code = status.HTTP_406_NOT_ACCEPTABLE, detail = "Já existe um usuário com email cadastrado")
...
async def get_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        result = await session.execute(query)
        usuario: List[UsuarioSchemaArtigos] = result.scalars().unique().one_or_none()
...
@router.put('/{usuario_id}', response_model = UsuarioSchemaBase, status_code = status.HTTP_201_CREATED)
...
async def delete_usuario(usuario_id: int, db: AsyncSession = Depends(get_session)):
    async with db as session:
        ...
        if usuario_del:
            ...
            await session.commit()
            return  # 204 será retornado automaticamente
        else:
            raise HTTPException(
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), db: AsyncSession = Depends(get_session)):
    ...
    return JSONResponse(
            content = {"access_token": criar_token_acesso(sub = usuario.id), 
                "token_type": "bearer"},
                status_code = status.HTTP_200_OK)
